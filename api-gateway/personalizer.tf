...
resource "aws_api_gateway_api_key" "api_key" {
  name        = "personalizer_api_key"
  description = "API key for accessing personalizer routes"
  enabled     = true
}

resource "aws_api_gateway_usage_plan" "usage_plan" {
  name = "personalizer_usage_plan"

  api_stages {
    api_id = aws_apigatewayv2_api.http_api.id
    stage  = aws_apigatewayv2_stage.api_gateway_stage.name
  }
}
